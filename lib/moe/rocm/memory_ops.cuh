...
struct W13Layout {
    ...
    ValueProjectionOffsetBytes(unsigned dim, unsigned inter_dim) {
        return dim * inter_dim * sizeof(Scalar);
    }

    TAL_HOST_DEVICE static constexpr unsigned
    ScaleProjectionOffsetBytes(unsigned dim, unsigned inter_dim) {
        return (dim / kScaleBlockSize) * (inter_dim / kScaleBlockSize) *
               sizeof(float);
    }
    ...
    __device__ float LoadScale(unsigned tid, unsigned scale_offset = 0);
...
enum class MxFp4TileShape : unsigned {
    ...
    kM64N256,
    // Eight-wave M64 x N256 (per projection): every wave spans M64 and owns
    // one N32 slice. Gate and up together are M64 x N64 per wave, matching
    // FlyDSL's bounded-M stage-1 ownership.
    kM64N256W8,
};
...
struct MxFp4WeightLayoutSelector<MxFp4TileShape::kN256, kNumWarps> {
    ...
    static constexpr unsigned kWaveTileM = 32;
...
template <unsigned kNumWarps>
struct MxFp4WeightLayoutSelector<MxFp4TileShape::kM64N256W8, kNumWarps> {
    static constexpr unsigned kTileM = 64;
    static constexpr unsigned kWaveTileM = 64;
    static constexpr unsigned kGroupN = 256;
    static constexpr unsigned kTileK = 256;
    static constexpr unsigned kLoadGlobal = 2;
    static constexpr unsigned kWaveTileN = 32;
    ...
    __host__ __device__ static constexpr unsigned
    ValueOffsetBytes(unsigned wid, unsigned fragment, unsigned stride_n) {
        return (wid * kWaveTileN + fragment * 16) * stride_n / 2;
    }
    ...
    __host__ __device__ static constexpr unsigned
    N32Offset(unsigned wid, unsigned) {
        return wid;
    }
};
...
struct MxFp4WeightLayout {
    ...
    static constexpr unsigned kTileM = LayoutSelector::kTileM;
    static constexpr unsigned kWaveTileM = LayoutSelector::kWaveTileM;
    ...
    TAL_HOST_DEVICE static constexpr unsigned
    ScaleProjectionOffsetBytes(unsigned dim, unsigned inter_dim) {
        return dim * inter_dim / kRowGroupSize;
    }
...
#pragma unroll
    for (unsigned fragment = 0; fragment < kLoadGlobal; ++fragment) {
        const unsigned voffset =
            ...
            lane_k_offset + k_offset + value_offset;
...
MxFp4WeightLayout<kNumWarps_, kLayout_>::LoadScale(unsigned wid, unsigned wtid,
    ...
    if constexpr (kLayout_ == MxFp4TileShape::kN128) {
        ...
        // component is carried by s_offset_ or folded into the W2 resource
        // base.
        const unsigned n32 = LayoutSelector::N32Offset(wid, n32_pair);
        const unsigned word = n32 * k256_blocks * kWarpSize + wtid;
        return scales_.template LoadU32<BufferResource::kNone>(
            word * sizeof(unsigned) + scale_offset, s_offset_);
    }
}
