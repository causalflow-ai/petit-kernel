...
// two 16-token groups.
template <unsigned kTokenBatch_, unsigned kNumWarps_, unsigned kGroupDim_>
struct InputLayout {
    static constexpr unsigned kTokenBatch = kTokenBatch_;
    static constexpr unsigned kNumWarps = kNumWarps_;
    static constexpr unsigned kGroupDim = kGroupDim_;
    static constexpr unsigned kThreads = kNumWarps * kWarpSize;
...
template <unsigned kTokenBatch_, unsigned kNumWarps_, unsigned kGroupDim_>
__device__ void InputLayout<kTokenBatch_, kNumWarps_, kGroupDim_>::Initialize(
    const void *value_ptr, unsigned value_range, const void *scale_ptr,
    unsigned scale_range, unsigned dim) {
    values_.v = {
        .ptr = reinterpret_cast<uintptr_t>(value_ptr),
...
template <unsigned kTokenBatch_, unsigned kNumWarps_, unsigned kGroupDim_>
__device__ void InputLayout<kTokenBatch_, kNumWarps_, kGroupDim_>::FetchAsync(
    unsigned *shm_x, unsigned wid, unsigned wtid,
    const unsigned tokens[kTokenBatch]) {
    ...
    for (int i = 0; i < kTokenBatch; i++) {
        const unsigned src_off =
            tokens[i] * dim_ + values_offset_bytes_ + wtid * sizeof(unsigned);
        values_.LoadLds<BufferResource::kNone, sizeof(unsigned), 0>(lds_ptr,
                                                                    src_off, 0);
        lds_ptr += kWarpSize;
    }
...
template <unsigned kTokenBatch_, unsigned kNumWarps_, unsigned kGroupDim_>
__device__ void InputLayout<kTokenBatch_, kNumWarps_, kGroupDim_>::FetchToRegs(
    uint4 regs[kActivationFragments], const unsigned *shm_x,
    unsigned wtid) const {
...
template <unsigned kTokenBatch_, unsigned kNumWarps_, unsigned kGroupDim_>
__device__ void
InputLayout<kTokenBatch_, kNumWarps_, kGroupDim_>::FetchToRegsFP4(
    uint4 regs[kActivationFragments], const unsigned *shm_x,
    unsigned wtid) const {
    ...
    for (int i = 0; i < 2; i++) {
        for (unsigned j = 0; j < kActivationFragments; j++) {
            r[i * kActivationFragments + j] = x[i * kWarpSize * 2 + j * 4];
        }
    }
}
...
template <unsigned kTokenBatch_, unsigned kNumWarps_, unsigned kGroupDim_>
__device__ void
InputLayout<kTokenBatch_, kNumWarps_, kGroupDim_>::FetchScaleAsync(
    float *shm_scale_x, unsigned wid, unsigned wtid, const uint2 token_select,
    unsigned m) {
    (void)wtid;
    auto lds_ptr = (__attribute__((address_space(3))) unsigned *)shm_scale_x +
                   wid * kWarpSize;
    ...
    scales_offset_bytes_ += m * (kGroupDim / kScaleBlockSize) * sizeof(float);
}

template <unsigned kTokenBatch_, unsigned kNumWarps_, unsigned kGroupDim_>
__device__ float4
InputLayout<kTokenBatch_, kNumWarps_, kGroupDim_>::FetchScaleToReg(
    const float *shm_scale_x, unsigned tid) const {
    float4 r{0, 0, 0, 0};
