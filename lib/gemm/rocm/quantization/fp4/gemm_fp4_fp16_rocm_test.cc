...
void GemmFp4Fp16Test::CopyAndCompareOutput(GemmMPTestData *ctx) const {
    ...
    for (unsigned i = 0; i < ctx->m() * ctx->n(); ++i) {
        if (dequant_type_ == DataType::kDataTypeFp16) {
            ...
            EXPECT_NEAR(of, rf, std::min<float>(2e-2, fabs(rf) * 0.01f))
...
Fp4NK(int features, MatmulPipeline pipeline, MatmulMfmaType mfma_type,
...
TEST_F(GemmFp4Fp16Test, TestGemm16x64x512_16x48x512) {
    TestGemm(16, 64, 512, 1.0f,
             Fp4NK(MatmulFeatures::kMatmulFeatures_Grid |
                       MatmulFeatures::kMatmulFeatures_HighPrecision,
                   MatmulPipeline::kMatmulPipeline_1,
                   MatmulMfmaType::kMatmulMfmaTypeFp16, 1, 4, 32, 1, 2, 1));
}
TEST_F(GemmFp4Fp16Test, TestGemm32x64x512_32x48x512) {
    TestGemm(32, 64, 512, 1.0f,
             Fp4NK(MatmulFeatures::kMatmulFeatures_Grid |
                       MatmulFeatures::kMatmulFeatures_HighPrecision,
                   MatmulPipeline::kMatmulPipeline_1,
...
TEST_F(GemmFp4Fp16Test, TestGemm32x64x512_16x48x512) {
    TestGemm(32, 64, 512, 1.0f,
             Fp4NK(MatmulFeatures::kMatmulFeatures_Grid |
                       MatmulFeatures::kMatmulFeatures_HighPrecision,
                   MatmulPipeline::kMatmulPipeline_1,
...
TEST_F(GemmFp4Fp16Test, TestGemm16x64x512_16x48x256) {
    TestGemm(16, 64, 512, 1.0f,
             Fp4NK(MatmulFeatures::kMatmulFeatures_Grid |
                       MatmulFeatures::kMatmulFeatures_HighPrecision,
                   MatmulPipeline::kMatmulPipeline_1,
...
TEST_F(GemmFp4Fp16Test, TestGemm32x64x512_32x48x256) {
    TestGemm(32, 64, 512, 1.0f,
             Fp4NK(MatmulFeatures::kMatmulFeatures_Grid |
                       MatmulFeatures::kMatmulFeatures_HighPrecision,
                   MatmulPipeline::kMatmulPipeline_1,
...
TEST_F(GemmFp4Fp16Test, TestGemm32x64x512_16x48x256) {
    TestGemm(32, 64, 512, 1.0f,
             Fp4NK(MatmulFeatures::kMatmulFeatures_Grid |
                       MatmulFeatures::kMatmulFeatures_HighPrecision,
                   MatmulPipeline::kMatmulPipeline_1,
