...
static constexpr unsigned kNvRowGroupSize = 16;
...
static constexpr float kDefaultGlobalScale = 1.0f;
...
template <class Element, unsigned kM, unsigned kN> struct DeviceContext {
    ...
    uint4 d_scales[kM * kN / kNvRowGroupSize / kVecSize];
    uint4 d_reference[kM * kN / kOutVecSize];
    uint4 d_petit_weights[kM * kN / kPackFactor / kQuantVecSize];
    uint4 d_petit_scales[kM * kN / kNvRowGroupSize / kVecSize];
...
DeviceContext<Element, kM, kN>::PrepareDevice() {
    ...
    std::vector<ScaleType> h_scales(kM * kN / kNvRowGroupSize);
...
void DeviceContext<Element, kM, kN>::CompareOutputsFromDevice() const {
    ...
    for (unsigned i = 0; i < kM * kN; ++i) {
        ASSERT_EQ(h_reference[i], h_petit_output[i])
...
struct ExhaustiveDeviceContext {
    ...
    uint4 d_reference[kM * kN / kOutVecSize];
    uint4 d_petit_weights[kM * kN / kPackFactor / kQuantVecSize];
    uint4 d_petit_scales[kM * kN / kScaleRowGroupSize / kVecSize];
    uint4 d_output[kM * kN / kOutVecSize];

    template <class GenerateQweight, class GenerateScale>
    static ExhaustiveDeviceContext *PrepareDevice(GenerateQweight generate_q,
                                                  GenerateScale generate_scale);

    void CompareReferenceAndOutputFromDevice() const;

    template <class GenerateOutput>
    void ValidateOutputsFromDevice(GenerateOutput generate_output) const;
};

template <class Element, unsigned kN_, unsigned kScaleRowGroupSize_>
template <class GenerateQweight, class GenerateScale>
ExhaustiveDeviceContext<Element, kN_, kScaleRowGroupSize_> *
ExhaustiveDeviceContext<Element, kN_, kScaleRowGroupSize_>::PrepareDevice(
    GenerateQweight generate_q, GenerateScale generate_scale) {
    ExhaustiveDeviceContext<Element, kN_, kScaleRowGroupSize_> *d_ctx;
    CheckHIPStatus(hipMalloc(
        &d_ctx,
        sizeof(ExhaustiveDeviceContext<Element, kN_, kScaleRowGroupSize_>)));

    std::vector<unsigned> h_qweights(kM * kN / kPackFactor);
    std::vector<uint8_t> h_scales(kM * kN / kScaleRowGroupSize);

    for (unsigned i = 0; i < kM * kN / kPackFactor; ++i) {
        unsigned row = i % (kM / kPackFactor);
        unsigned col = i / (kM / kPackFactor);

        unsigned qweight =
            std::clamp(generate_q(row, col), kQweightBitMin, kQweightBitMax);
        unsigned vec = qweight * 0x11111111;
        h_qweights[i] = vec;
    }

    for (unsigned i = 0; i < kM * kN / kScaleRowGroupSize; ++i) {
        unsigned row = i % (kM / kScaleRowGroupSize);
        unsigned col = i / (kM / kScaleRowGroupSize);
        h_scales[i] = static_cast<uint8_t>(generate_scale(row, col));
    }
    ...
    CheckHIPStatus(hipMemcpy(d_ctx->d_weights_quant, h_qweights.data(),
                             ...
                             hipMemcpyHostToDevice));
    CheckHIPStatus(hipMemcpy(d_ctx->d_scales, h_scales.data(),
                             h_scales.size() * sizeof(uint8_t),
                             hipMemcpyHostToDevice));
    return d_ctx;
}

template <class Element, unsigned kN_, unsigned kScaleRowGroupSize_>
void ExhaustiveDeviceContext<Element, kN_, kScaleRowGroupSize_>::
    CompareReferenceAndOutputFromDevice() const {
    std::vector<Element> h_reference(kM * kN), h_petit_output(kM * kN);
    CheckHIPStatus(hipMemcpy(h_reference.data(), d_reference,
                             h_reference.size() * sizeof(Element),
                             hipMemcpyDeviceToHost));
    ...
    for (unsigned i = 0; i < kM * kN; ++i) {
        ASSERT_EQ(h_reference[i].to_bits(), h_petit_output[i].to_bits())
...
template <class GenerateOutput>
void ExhaustiveDeviceContext<Element, kN_, kScaleRowGroupSize_>::
    ValidateOutputsFromDevice(GenerateOutput generate_output) const {
    ...
    for (unsigned i = 0; i < kM * kN / kScaleRowGroupSize; ++i) {
        unsigned row = i % (kM / kScaleRowGroupSize);
        unsigned col = i / (kM / kScaleRowGroupSize);
        float reference_fp32 = generate_output(row, col);
        Element reference = Element::from_fp32(reference_fp32);
        ASSERT_EQ(reference, h_petit_output[i * kScaleRowGroupSize])
            << "Output and reference differ at (" << row << ", " << col << ")";
    }
}

using Fp8ScaleType = tests::cpu_numeric::fp8_e4m3_t;
static constexpr unsigned kNvScaleBitMin =
    std::numeric_limits<Fp8ScaleType>::denorm_min().to_bits();
static constexpr unsigned kNvScaleBitMax =
    std::numeric_limits<Fp8ScaleType>::max().to_bits();

static unsigned GenerateQweight(unsigned i) {
    return std::clamp(i, kQweightBitMin, kQweightBitMax);
}

static Fp8ScaleType GenerateNvScale(unsigned i) {
    i = std::clamp(i, kNvScaleBitMin, kNvScaleBitMax);
    return Fp8ScaleType::from_bits(static_cast<uint8_t>(i));
}

static float GenerateNvOutput(unsigned i_qweight, unsigned i_scale) {
    unsigned qweight = GenerateQweight(i_qweight);
    Fp8ScaleType scale = GenerateNvScale(i_scale);
...
void RunPetitDequant(Context *d_ctx, float global_scale, DataType out_type,
                     RepackScalesFn repack_scales,
                     DequantPetitFn dequant_petit) {
    ...
    repack_scales(reinterpret_cast<unsigned *>(d_ctx->d_petit_scales),
                  reinterpret_cast<const unsigned *>(d_ctx->d_scales), kM, kN,
                  nullptr);

    ASSERT_EQ(dequant_petit(
                  reinterpret_cast<unsigned *>(d_ctx->d_output),
                  reinterpret_cast<const unsigned *>(d_ctx->d_petit_weights),
                  reinterpret_cast<const unsigned *>(d_ctx->d_petit_scales),
                  global_scale, out_type, kM, kN),
...
class MxFp4DequantTest : public ::testing::Test {
  public:
    template <class Element> void TestAllCombinations(DataType out_type) const {
        using Context = MxExhaustiveDeviceContext<Element>;
        constexpr unsigned kM = Context::kM;
        constexpr unsigned kN = Context::kN;
        static_assert(kN >= kMxScaleCount, "kN must cover all MX scale values");
        ...
        ASSERT_EQ(
            DequantMxFp4(
                reinterpret_cast<unsigned *>(d_ctx->d_reference),
                reinterpret_cast<const unsigned *>(d_ctx->d_weights_quant),
                reinterpret_cast<const unsigned *>(d_ctx->d_scales),
                kDefaultGlobalScale, out_type, kM, kN),
            0);

        RunPetitDequant(d_ctx, kDefaultGlobalScale, out_type,
                        RepackMxFp4ToPetitFp4Scales, DequantPetitMxFp4);
...
class ExhaustiveFp4DequantTest : public ::testing::Test {
  public:
    template <class Element> void TestAllCombinations(DataType out_type) const {
        using Context = NvExhaustiveDeviceContext<Element>;
        auto d_ctx = Context::PrepareDevice(
            [](unsigned row, unsigned) {
                unsigned group_i = row * kPackFactor / kNvRowGroupSize;
                return GenerateQweight(group_i);
            },
            [](unsigned, unsigned col) {
                return GenerateNvScale(col).to_bits();
            });

        RunPetitDequant(d_ctx, kDefaultGlobalScale, out_type,
                        RepackNvFp4ToPetitFp4Scales, DequantPetitFp4);

        d_ctx->ValidateOutputsFromDevice([](unsigned row, unsigned col) {
            return GenerateNvOutput(row, col);
        });
        CheckHIPStatus(hipFree(d_ctx));
    }
};
