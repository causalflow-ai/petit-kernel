...
template <class Config> struct MatrixALayout {
    ...
    FetchRegisters(uint4 va[Config::kWarpTilesM][kReadBatchA],
...
template <class Config> struct ShmBuf {
    ...
    using Layout = union {
        ...
        uint2 result[kGroupM * kGroupN / 4];
...
__device__ void MatrixALayout<Config>::FetchRegisters(
    uint4 va[Config::kWarpTilesM][kReadBatchA], const Shm &__restrict__ shm,
    unsigned tile_idx_m, unsigned tile_idx_k, unsigned wtid) {
    for (int m = 0; m < Config::kWarpTilesM; m++) {
        for (int j = 0; j < kReadBatchA; j++) {
            auto shm_coord =
                Config::ReadShmCoordA(m + tile_idx_m, tile_idx_k, j, wtid);
